...
const Customer = () => {
    ...
    const contract = new ethers.Contract("0x1D7600Ab6E9a88599315DE7081A0D124384E0E24",nftabi,signer || provider);
  ...
  const[productowner,setProductOwner] = React.useState<String>("");
    ...
    const mintnft = async () => {
        try{const tx = await contract.safeMint(
            "Product 1",
            "Material 1",
            "Origin 1",
    ...
    const getProductDetails = async (tokenid: String) => {
        const contract = new ethers.Contract(
          "0x1D7600Ab6E9a88599315DE7081A0D124384E0E24",
          nftabi,
          signer || provider
        );
  ...
  return(
    <div className="flex flex-row h-screen w-screen justify-center items-center">
        <div className="flex flex-col h-screen w-1/2 justify-center items-center">
            ...
            <ConnectButton/>
            <button className="bg-blue-600 text-white font-bold py-2 px-4 mx-2 rounded-xl mt-4" onClick={()=>mintnft()}>
                Mint
            </button>
        </div>
        <div className="flex flex-col h-screen w-1/2 justify-center items-center">
          ...
          onClick={() => getProductDetails(tokenid)}>
            ...
            {address?.toString().toLowerCase()  === productowner.toString().toLowerCase() ? (
          <div className="flex flex-col mt-7">
            ...
            {
            productStatus == "2" ? (
                <button className="bg-blue-600 text-white font-bold py-2 px-4 mx-2 rounded-xl mt-4" onClick={()=>confirmDelivery()}>
                    Confirm Delivery
                </button>
            ):null
        }
